...
class AiChatActivity : AppCompatActivity() {
    ...
    private var recorderInitialized: Boolean = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (devId.isNullOrEmpty()) {
            Log.e(TAG, "Device ID is required to initialize AI Stream.")
    ...
    private fun initAiStream() {
        ...
        aiStream = ThingAIOS.getInstance().getAiStream().apply {
    ...
    private fun interruptPreviousTts() {
        aiStream?.stopPlayAudio()

        if (selectedImageUri != null && !mCurrentEventId.isNullOrEmpty()) {
            Log.d(TAG, "interruptPreviousTts: image staged with event ${mCurrentEventId}, skip chatBreak")
            return
        }

        val eventId = mCurrentEventId
        val session = currentSessionId
        if (!eventId.isNullOrEmpty() && !session.isNullOrEmpty()) {
            Log.i(TAG, "Previous event $eventId still active, sending chatBreak")
            mCurrentEventId = null
            aiStream?.sendEventChatBreak(eventId, session, null, object : StreamResultCallback {
                override fun onSuccess() {
                    Log.i(TAG, "ChatBreak success for event: $eventId")
                }
                override fun onError(errorCode: Int, errorMessage: String) {
                    Log.w(TAG, "ChatBreak failed for event $eventId: $errorMessage")
                }
            })
        }
    }
    ...
    private fun connectToAiStream() {
        if (aiStream?.isConnected(Constants.ClientType.DEVICE, deviceId) == true) {
            Log.i(TAG, "Already connected to device: $deviceId")
        ...
        isConnecting = true
        tvStatus.text = "Status: Connecting to $deviceId..."
    ...
    private fun createNewSession() {
        ...
        // 使用设备维度创建会话，与 AIStreamActivity 一致
        aiStream?.createSession(deviceId, "", null, object : SessionCallback {
            override fun onSuccess(
                ...
                Log.i(TAG, "createSession onSuccess: $sessionId, sendDataCodes: $sendDataCodes, revDataCodes: $revDataCodes")
                // State change will be handled by listener
            }
    ...
    private fun isStreamConnected(): Boolean =
        aiStream?.isConnected(Constants.ClientType.DEVICE, deviceId) == true
    ...
    private fun proceedWithAudioRecordingInternal(eventIdToUse: String) {
        ...
        aiStream?.startRecordAndSendAudioData(
            ...
            object : StreamResultCallback {
                override fun onSuccess() {
                    Log.d(TAG, "startRecordAndSendAudioData success for event: $eventIdToUse")
                }
                ...
                override fun onError(code: Int, message: String) {
                    ...
                    if (eventIdToUse == mCurrentEventId && selectedImageUri == null) {
                        ...
                        Log.w(TAG, "Audio part of image event failed to start recording. Image event $mCurrentEventId remains.")
                    }
                    runOnUiThread {
                        dismissRecordingPopup()
                        hideVoiceRecordingUI()
    ...
    private val aiStreamListener = object : ThingAiStreamListener {
        ...
        override fun onImageReceived(imageData: StreamImage) {
            ...
            if (imageData.streamFlag == Constants.StreamFlag.END) {
                Log.i(TAG, "Received image end packet: filePath=${imageData.filePath}, format=${imageData.format}, streamFlag=${imageData.streamFlag}")
                return
            }
            ...
            Log.i(TAG, "Image received: ${imageData.payload!!.size} bytes, filePath=${imageData.filePath}, imageUrl=${imageData.imageUrl}, format=${imageData.format}, streamFlag=${imageData.streamFlag}")
            
            // 优先使用 filePath，如果没有则使用 imageUrl
            val imagePath = imageData.filePath?.takeIf { it.isNotEmpty() } 
                ?: imageData.imageUrl?.takeIf { it.isNotEmpty() }
            ...
            if (imagePath != null) {
                ...
                Log.w(TAG, "Image received but no filePath or imageUrl available")
            }
        }
        ...
        override fun onEventReceived(event: StreamEvent) {
            Log.i(TAG, "Event received: eventId=${event.eventId}, type=${event.eventType}")
            if (event.eventType == Constants.EventType.END) {
                Log.i(TAG, "Event ${event.eventId} ended from server")
                if (event.eventId == mCurrentEventId) {
                    mCurrentEventId = null
                }
            }
        }
    ...
    private fun parseAndDisplayReceivedText(jsonText: String) {
        try {
            ...
            // 先尝试获取 data 作为 JSONObject
            var data = jsonObject.optJSONObject("data")
            
            // 如果 data 不是 JSONObject，尝试作为字符串处理（可能是图片 URL）
            if (data == null) {
                ...
                if (dataString.isNotEmpty()) {
                    ...
                    if (dataString.startsWith("http://", ignoreCase = true) || 
                        ...
                        Log.i(TAG, "Received image URL in data field: $dataString")
                        // 尝试替换占位图，如果没有占位图则添加新消息
                        if (!tryReplacePlaceholderImage(bizId, dataString)) {
                            addMessage(
                                ChatMessage(
                                    imageUrl = dataString,
                                    isSentByUser = false,
                        ...
                        // data 是字符串但不是 URL，作为文本处理
                        Log.w(TAG, "Data field is string but not URL: $dataString")
                        addMessage(
                            ChatMessage(
                                text = dataString,
                                isSentByUser = false,
                    ...
                    // data 字段不存在或为空
                    Log.e(TAG, "No data object or string in received JSON: $jsonText")
                    addMessage(
                        ChatMessage(
                            text = jsonText,
                            isSentByUser = false,
    ...
    private fun handleNlgData(data: JSONObject, bizId: String, eof: Int) {
        ...
        if (content.isNotEmpty()) {
            // 检查 content 是否为图片生成通知（JSON 格式）
            if (tryHandleImageGenerationNotification(content, bizId)) {
                // 已处理为图片生成通知，不作为普通文本显示
                Log.d(TAG, "Handled as image generation notification: $content")
            } else {
                // 普通文本消息
                var appended = false
        ...
        data.optJSONArray("images")?.let { imagesArray ->
            for (i in 0 until imagesArray.length()) {
                imagesArray.optJSONObject(i)?.optString("url")?.takeIf { it.isNotEmpty() }
                    ?.let { imageUrl ->
                        // 尝试替换占位图，如果没有占位图则添加新消息
                        if (!tryReplacePlaceholderImage(bizId, imageUrl)) {
                            addMessage(
                                ChatMessage(
                                    imageUrl = imageUrl,
                                    isSentByUser = false,
    ...
    /**
     * 尝试解析并处理图片生成通知
     * 格式：{"custom_type": "image", "image_count": N}
     * @return true 如果成功处理为图片生成通知，false 否则
     */
    private fun tryHandleImageGenerationNotification(content: String, bizId: String): Boolean {
        try {
            // 尝试解析为 JSON
            ...
            val customType = jsonObject.optString("custom_type", "")
            
            if (customType == "image") {
                val imageCount = jsonObject.optInt("image_count", 1)
                Log.i(TAG, "Detected image generation notification: count=$imageCount, bizId=$bizId")
                ...
                for (i in 0 until imageCount) {
                    addMessage(
                        ChatMessage(
                            text = "图片生成中...",
                            isSentByUser = false,
                            messageType = ChatMessage.MessageType.NLG_IMAGE_PLACEHOLDER,
                            bizId = bizId,
                            isPlaceholder = true,
                            placeholderIndex = i
                        )
            ...
            // 不是有效的 JSON，作为普通文本处理
            Log.d(TAG, "Content is not a valid JSON for image notification: ${e.message}")
        }
        return false
    }
    ...
    /**
     * 尝试用真实图片 URL 替换占位图
     * @return true 如果成功替换，false 如果没有找到占位图
     */
    private fun tryReplacePlaceholderImage(bizId: String, imageUrl: String): Boolean {
        // 查找对应 bizId 的第一个未替换的占位图
        val placeholderIndex = messageList.indexOfFirst { msg ->
            msg.bizId == bizId && 
            msg.messageType == ChatMessage.MessageType.NLG_IMAGE_PLACEHOLDER &&
            msg.isPlaceholder
        }
        
        if (placeholderIndex != -1) {
            val placeholder = messageList[placeholderIndex]
            // 替换占位图为真实图片
            messageList[placeholderIndex] = ChatMessage(
                imageUrl = imageUrl,
                isSentByUser = false,
                messageType = ChatMessage.MessageType.NLG_IMAGE,
                bizId = bizId,
            ...
            Log.i(TAG, "Replaced placeholder at index $placeholderIndex with image: $imageUrl")
            return true
        }
        return false
    }
