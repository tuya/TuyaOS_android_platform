...
class MainActivity : AppCompatActivity() {
        ...
        override fun onMQTTStatusChanged(status: Int) {
            ...
            if (status == IoTSDKManager.STATUS_MQTT_ONLINE) {
                ...
                if (!hasInitFrame) {
                    ...
                    ThingFrameOS.getInstance().photoFrame.initFrameZone(
                        config,
                        object : ApiCallback<Void> {
                            ...
                            override fun onFailure(errorCode: String, errorMessage: String) {
                                Log.e(
                                    TAG,
                                    "onFailure() called with: errorCode = [$errorCode], errorMessage = [$errorMessage]"
                                )
                                hasInitFrame = false
                            }
                        })
    ...
    private val mUpgradeCallback = object : UpgradeEventCallback {
        override fun onUpgradeInfo(info: FirmwareUpgradeInfo) {
            ...
            Toast.makeText(
                this@MainActivity,
                "收到固件升级通知 v${info.version}，开始自动升级",
                Toast.LENGTH_SHORT
            ).show()
        ...
        override fun upgradeFileDownloadFinished(resultCode: Int, file: String) {
            ...
            tvDataConsole.text = dataConsole.toString()
            if (resultCode == 0) {
                Toast.makeText(this@MainActivity, "升级文件下载成功，准备安装...", Toast.LENGTH_LONG)
                    .show()
                ...
                Thread {
                    ioTSDKManager.reportUpgradeStatus(
                        0,
                        UpgradeStatus.TUS_UPGRADE_ERROR_LOW_BATTERY
                    )
                }.start()
            }
        }
    ...
    private fun printLn() {
        ...
        for (info in photoInfos) {
            Log.d(
                TAG,
                "realFn: ${info.realFn}, title: ${info.title}, type: ${info.fileType},prefix: ${info.prefix}, centerX: ${info.centerX}, localPath: ${info.localPath}"
            )
        }
    }
    ...
    private fun installApk(apkPath: String) {
        ...
        if (!apkFile.exists()) {
            ...
            runOnUiThread {
                Toast.makeText(this, "安装失败：APK 文件不存在", Toast.LENGTH_SHORT).show()
            }
            return
        }
        ...
        try {
            ...
            val intent =
                Intent("com.tuya.smartai.demo.INSTALL_COMPLETE").apply { setPackage(packageName) }
            val pendingIntent = android.app.PendingIntent.getBroadcast(
                this, sessionId, intent, android.app.PendingIntent.FLAG_UPDATE_CURRENT
            )
            ...
            session?.abandon()
            runOnUiThread {
                Toast.makeText(this, "安装异常: ${e.message}", Toast.LENGTH_LONG).show()
            }
        } finally {
            try {
                os?.close()
    ...
    private fun initSDK() {
        ...
        if(uuid == "xxxxx"){
            uuid2   //这里防止未配置 uuid 与 key，直接运行，强制编译不通过。配置uuid后删除即可
        }
    ...
    private fun sendEmotionDP(mode: String) {
        ...
        val event1 = DPEvent(
            2,
            DPEvent.Type.PROP_RAW.toByte(),
            JSONObject.toJSONString(valueMap).toByteArray(),
            timestamp
        )
        ...
        if (modeIndex in emotionNames.indices) {
            Toast.makeText(this, "已发送表情: ${emotionNames[modeIndex]}", Toast.LENGTH_SHORT)
                .show()
        }
    }
    ...
    fun onClick(v: View) {
        when (v.id) {
                ...
                ioTSDKManager.deviceApi.getWeather(codes2, object : ApiCallback<String> {
                    override fun onSuccess(result: String) {
                        ...
                        runOnUiThread {
                            ...
                            tvDataConsole.text = dataConsole.toString()
                            Toast.makeText(this@MainActivity, "天气获取成功", Toast.LENGTH_SHORT)
                                .show()
                        }
                    }
            ...
            R.id.get_weather_default -> {
                ioTSDKManager.deviceApi.getWeatherDefault(object :
                    ApiCallback<com.tuya.smartai.iot_sdk.model.WeatherInfo> {
                    override fun onSuccess(weatherInfo: com.tuya.smartai.iot_sdk.model.WeatherInfo) {
                        ...
                        runOnUiThread {
                            ...
                            weatherInfo.getTemp(0)
                                ?.let { weatherText.append("今天温度: ${it}°C\n") }
                            weatherInfo.getHumidity(0)
                                ?.let { weatherText.append("今天湿度: ${it}%\n") }
                            weatherInfo.getConditionNum(0)
                                ?.let { weatherText.append("今天天气状况: $it\n") }
                ...
                runOnUiThread {
                    ...
                    try {
                        ...
                        if (json != null) {
                            ...
                            json.getInteger("upgradeType")
                                ?.let { upgradeInfo.append("升级类型: $it\n") }
                            ...
                            json.getString("httpsUrl")
                                ?.let { upgradeInfo.append("HTTPS下载地址: $it\n") }
                            json.getLong("execTime")?.let { upgradeInfo.append("执行时间: $it\n") }

                            json.getString("diffUrl")?.let { diffUrl ->
                                upgradeInfo.append("\n--- 差分包信息 ---\n")
                                upgradeInfo.append("差分地址: $diffUrl\n")
                                json.getString("diffSize")
                                    ?.let { upgradeInfo.append("差分包大小: $it\n") }
                                json.getString("diffMd5")
                                    ?.let { upgradeInfo.append("差分包MD5: $it\n") }
                            }
                        } else {
                    ...
                    tvDataConsole.text = dataConsole.toString()
                    Toast.makeText(this@MainActivity, "固件升级信息获取成功", Toast.LENGTH_SHORT)
                        .show()
                }
            }
    ...
    private fun chooseNewImage(realFn: String, filePath: String, next: Boolean) {
        ...
        tvPhotoCount.text =
            if (photoInfos.isNotEmpty()) "展示图片: ${index + 1}/${photoInfos.size}" else "展示图片: 0/0"
        currentRealFn = fn
    ...
    private fun initFrameImageUpdateListener() {
        ThingFrameOS.getInstance().photoFrame.registerMediaUploadListener(object :
            IPhotoFrameMediaUpdateListener {
            ...
            override fun onMediaAppUploaded(event: UploadedEventData) {
                Log.e(
                    TAG,
                    "onImageAppUploaded() called with: sid = [${event.sid}], fileName = [${event.realFn}] ${event.userInfo}"
                )
            }
            ...
            override fun onMediaDeviceDownloaded(event: UploadedEventData, filePath: String) {
                Log.d(
                    TAG,
                    "onImageDeviceDownloaded() called with: sid = [${event.sid}], fileName = [${event.realFn}], filePath = [$filePath]"
                )
            ...
            override fun onMediaUploadFailed(sid: String, code: String, errorMessage: String) {
                Log.d(
                    TAG,
                    "onImageUploadFailed() called with: sid = [$sid], code = [$code], errorMessage = [$errorMessage]"
                )
                printLn()
            }
            ...
            override fun onMediaAppDelete(event: DeleteEventData) {
                Log.d(
                    TAG,
                    "onImageDelete() called with: sid = [${event.sid}], realFn = [${event.realFn}]"
                )
