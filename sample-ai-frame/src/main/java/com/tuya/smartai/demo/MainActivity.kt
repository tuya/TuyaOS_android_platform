...
class MainActivity : AppCompatActivity() {
    ...
    private val mIotCallback = object : IoTCallback {
        ...
        override fun onMQTTStatusChanged(status: Int) {
            ...
            if (status == IoTSDKManager.STATUS_MQTT_ONLINE) {
                ...
                tvMqttStatus.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.colorPrimary))
                val deviceId = ioTSDKManager.deviceId
                if (!hasInitFrame) {
                ...
                if (!ioTSDKManager.deviceId.isNullOrEmpty()) {
                    ...
                    tvMqttStatus.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.colorAccent))
                }
            }
        }
    ...
    private val mUpgradeCallback = object : UpgradeEventCallback {
        override fun onUpgradeInfo(info: FirmwareUpgradeInfo) {
            ...
            Toast.makeText(this@MainActivity, "收到固件升级通知 v${info.version}，开始自动升级", Toast.LENGTH_SHORT).show()
        ...
        override fun upgradeFileDownloadFinished(resultCode: Int, file: String) {
            ...
            tvDataConsole.text = dataConsole.toString()
            if (resultCode == 0) {
                Toast.makeText(this@MainActivity, "升级文件下载成功，准备安装...", Toast.LENGTH_LONG).show()
                Thread { installApk(file) }.start()
            } else {
                Toast.makeText(this@MainActivity, "升级文件下载失败: $resultCode", Toast.LENGTH_SHORT).show()
    ...
    private fun initSDK() {
        ...
        val params = IoTParams.Builder()
            ...
            .productId("figevyadbapdki1f") //aqcxhpklvzlblyl3
            .uuid("uuidf0e66ee59b1d5fc7")
            .authKey("bQJ5BeCKLEwkpXeg1R3wxkUt7B3aIMFW")
            .version("1.0.1")
            ...
            R.id.fetch_device_info_partial -> {
                ...
                ioTSDKManager.deviceApi.fetchDeviceInfo(fields, object : ApiCallback<String> {
                    ...
                    override fun onFailure(errorMessage: String?, msg: String?) {
                        Log.e(TAG, "fetchDeviceInfo partial failed: $errorMessage, $msg")
                        runOnUiThread {
                            Toast.makeText(this@MainActivity, "获取失败: ${errorMessage ?: msg ?: "unknown"}", Toast.LENGTH_SHORT).show()
                        }
                    }
                })
            }
                ...
                ioTSDKManager.deviceApi.getWeather(codes2, object : ApiCallback<String> {
                    override fun onSuccess(result: String) {
                        ...
                        runOnUiThread {
                            ...
                            tvDataConsole.text = dataConsole.toString()
                            Toast.makeText(this@MainActivity, "天气获取成功", Toast.LENGTH_SHORT).show()
                        }
                    }
                    ...
                    override fun onFailure(errorCode: String?, errorMessage: String?) {
                        ...
                        runOnUiThread {
                            Toast.makeText(this@MainActivity, "天气获取失败: ${errorMessage ?: "unknown"}", Toast.LENGTH_SHORT).show()
                        }
                    }
                })
            }
            R.id.get_weather_default -> {
                ioTSDKManager.deviceApi.getWeatherDefault(object : ApiCallback<com.tuya.smartai.iot_sdk.model.WeatherInfo> {
                    override fun onSuccess(weatherInfo: com.tuya.smartai.iot_sdk.model.WeatherInfo) {
                        ...
                        runOnUiThread {
                            ...
                            tvDataConsole.text = dataConsole.toString()
                            Toast.makeText(this@MainActivity, "默认天气获取成功", Toast.LENGTH_SHORT).show()
                        }
                    }
                    ...
                    override fun onFailure(errorCode: String?, errorMessage: String?) {
                        ...
                        runOnUiThread {
                            Toast.makeText(this@MainActivity, "默认天气获取失败: ${errorMessage ?: "unknown"}", Toast.LENGTH_SHORT).show()
                        }
                    }
                })
            }
        ...
        ThingApiHelper.request(apiParams, String::class.java, object : ApiCallback<String> {
            override fun onSuccess(result: String) {
                ...
                runOnUiThread {
                    ...
                    tvDataConsole.text = dataConsole.toString()
                    Toast.makeText(this@MainActivity, "固件升级信息获取成功", Toast.LENGTH_SHORT).show()
                }
            }
            ...
            override fun onFailure(errorCode: String?, errorMessage: String?) {
                ...
                runOnUiThread {
                    ...
                    tvDataConsole.text = dataConsole.toString()
                    Toast.makeText(this@MainActivity, "固件升级检查失败: ${errorMessage ?: "unknown"}", Toast.LENGTH_SHORT).show()
                }
            }
        })
    }
